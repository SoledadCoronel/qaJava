...
public class AddUserAllFieldsTest extends SetUp {
	...
	public void addUseAllFields() throws Exception {
		 ...
		 * 
		 * personas.setRandomUserName(); personas.setRandomLastName();
		 * personas.setRandomEmail(); personas.selectRol();
		 * 
		 * profile.agrandarForm(); log.info("Seteando un grupo");
		 * personas.setGrupo(); profile.agregarDatosLaborales();
		 * profile.agregarDatosPersonales(); profile.agregarTelefono();
		 * profile.agregarRedesSociales(); js.executeScript("scroll(0, 250);");
		 * Thread.sleep(3000); log.info("Grabando el usuario");
