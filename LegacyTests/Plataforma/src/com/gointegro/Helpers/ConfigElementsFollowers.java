package com.gointegro.Helpers;

/**
 * Datos utilizados para followers
 * 
 * @author gustavomoreira
 *
 */

public class ConfigElementsFollowers {
	
	/** Datos */
	
	//1 sigue a 2
	
	private static String follower1 = "diogo.darosa@gointegro.com";
	
	private static String follower2 = "constanza.luciani@gointegro.com";
	
	private static String follower1name = "Diogo da Rosa Hertzberg";
	
	private static String follower2name = "Constanza Luciani";
	
	private static String follower2profile = "constanza-luciani";
	
	private static String follower1pwd = "";
	
	private static String follower2pwd = "";
	
	/** Get */

	public static String getFollower1() {
		return follower1;
	}

	public static String getFollower2() {
		return follower2;
	}

	public static String getFollower1name() {
		return follower1name;
	}

	public static String getFollower2name() {
		return follower2name;
	}

	public static String getFollower2profile() {
		return follower2profile;
	}
	
	public static String getFollower1pwd() {
		return follower1pwd;
	}
	
	public static String getFollower2pwd() {
		return follower2pwd;
	}
	
	

}
